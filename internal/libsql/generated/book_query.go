...
func (_q *BookQuery) QueryVolume() *VolumeQuery {
...
func (_q *BookQuery) QueryChapters() *ChapterQuery {
...
func (_q *BookQuery) Clone() *BookQuery {
	...
	return &BookQuery{
		...
		withVolume:   _q.withVolume.Clone(),
...
func (_q *BookQuery) WithVolume(opts ...func(*VolumeQuery)) *BookQuery {
	query := (&VolumeClient{config: _q.config}).Query()
...
func (_q *BookQuery) WithChapters(opts ...func(*ChapterQuery)) *BookQuery {
	query := (&ChapterClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *BookQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Book, error) {
	...
	if query := _q.withVolume; query != nil {
		if err := _q.loadVolume(ctx, query, nodes, nil,
			func(n *Book, e *Volume) { n.Edges.Volume = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *BookQuery) loadVolume(ctx context.Context, query *VolumeQuery, nodes []*Book, init func(*Book), assign func(*Book, *Volume)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Book)
	for i := range nodes {
		if nodes[i].volume_books == nil {
			continue
		}
		fk := *nodes[i].volume_books
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "volume_books" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *BookQuery) loadChapters(ctx context.Context, query *ChapterQuery, nodes []*Book, init func(*Book), assign func(*Book, *Chapter)) error {
